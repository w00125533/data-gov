...
| 表名 | 默认主分类路径 | 默认标签 |
|------|----------------|----------|
| `ods_ue_signal` | `源数据 / CHR` | `ODS`, `覆盖`, `质量`, `射频`, `标识信息` |
| `ods_gnb_alarm` | `源数据 / 配置` | `ODS`, `BBU`, `电源`, `机房`, `质量` |
| `dwd_session_qos` | `网络 / 质量` | `DWD`, `速率`, `时延`, `丢包`, `保持`, `标识信息` |
| `dwd_ho_event` | `网络 / 移动` | `DWD`, `保持`, `接入`, `质量`, `标识信息` |
| `dws_cell_hourly` | `网络 / 覆盖` | `DWS`, `覆盖`, `话务`, `速率`, `保持`, `质量` |
| `dws_area_traffic` | `网络 / 话务` | `DWS`, `容量`, `速率`, `时延`, `活动信息` |
| `ads_cell_profile` | `网络 / 覆盖` | `ADS`, `覆盖`, `容量`, `质量`, `射频` |
| `ads_neighbor_pair` | `网络 / 移动` | `ADS`, `保持`, `质量`, `工参` |
| `eval_user_score` | `用户 / 业务信息` | `EVAL`, `覆盖`, `移动`, `业务信息`, `活动信息` |
| `eval_net_health` | `网络 / 质量` | `EVAL`, `话务`, `机房`, `业务信息` |

`ODS/DWD/DWS/ADS/EVAL` 属于“表层级”通用标签组，用于跨主分类过滤表生命周期层级；它们不进入主分类树，也不替代表节点上的 `layer/layer_priority` 属性。

分类是表的唯一主归属，标签是补充筛选属性；系统不会因为表属于某个二级分类而自动继承同名标签。同名或近义标签只在确实表达该表业务语义时绑定。示例数据中，`网络 / 覆盖` 下的 `dws_cell_hourly`、`ads_cell_profile` 绑定 `覆盖` 标签；`eval_net_health` 属于 `网络 / 质量`，不绑定 `覆盖` 标签，避免在“网络 + 覆盖”组合筛选时误命中质量类评估表。
...
- SQL 逻辑预览不自动覆盖 `Table.sql_logic`；第一版通过 SQL 导入抽屉的“确认应用”写入 Table 节点并追加 `Change`，左侧 SQL 逻辑区的“同步到表定义”按钮先保留为后续直接写回入口
...
- 分类过滤表：`MATCH (t:Table)-[:IN_CATEGORY]->(c:MetaCategory)`；`include_children=true` 时先展开目标分类后代再过滤。一级分类如 `网络` 命中其所有二级分类下的表；二级分类如 `网络 / 覆盖` 只命中该叶子分类下的表。
- 标签过滤表：`MATCH (t:Table)-[:TAGGED_WITH]->(tag:MetaTag)`；分类树过滤与标签过滤之间固定为 AND；多标签默认 `tag_match=all`，要求表拥有全部选中标签，`tag_match=any` 仅保留为兼容性扩展语义。前端不暴露“任一/全部”切换，所有标签组合筛选均按全部匹配发送。
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│ 左侧: 元数据管理       │ 中间: 查询结果表列表        │ 右侧: 表详细信息         │
├────────────────────────┼─────────────────────────────┼─────────────────────────┤
│ 搜索: 表名/字段/描述    │ [导出 YAML]                 │ dws_cell_hourly          │
│                        │                             │ 描述: 小区小时粒度汇总... │
│ [全部]      [+新建表] [管理分类]│ ★ dws_cell_hourly   │ [编辑表] [新建字段]       │
│  ▸ 网络                │   网络/覆盖 · DWS · 8 fields │ [预览 YAML] [查看血缘]   │
│  ▸ 用户                │                             │ [演化历史] [删除]         │
│  ▸ 业务                │   ads_cell_profile          │                         │
│  ▸ 源数据              │   网络/覆盖 · ADS · 6 fields │ ┌── 字段列表 ─────────┐  │
│  (默认收起，点击展开)   │                             │ │字段 │类型 │上游 │表达式│  │
│                        │   dwd_session_qos           │ │cell │STR │无   │...  │  │
│                        │   网络/质量 · DWD · 9 fields │ │avg_ │DBL │...  │AVG()│  │
│ 标签                   │                             │ └─────────────────────┘  │
│ [表层级:DWD] [覆盖]     │ [点击表行]: 显示右侧详情      │ [点击字段]: 打开字段抽屉   │
└────────────────────────┴─────────────────────────────┴─────────────────────────┘
```

`/metadata` 不再保留全局顶部搜索框、FastAPI/Neo4j/metadata 状态徽标；页面空间优先给左侧治理导航、中间表列表和右侧表详情。环境状态集中放到 `/health` 页面查看。

#### 功能详单

| 功能 | 入口 | 说明 |
|------|------|------|
| 分类浏览 | 左侧分类树 | 默认收起；点击大分类展开并过滤其全部二级分类下的表；点击二级分类只过滤该叶子分类下的表；表节点挂接在二级分类下，点击表节点打开右侧详情 |
| 搜索表 | 左侧搜索框 | 模糊匹配表名/字段名/描述 |
| 表详情 | 点击表行 | 右侧面板第一行单独显示表名和描述，第二行显示操作按钮；信息卡片中的“层级”显示主分类两级路径，如 `网络 / 覆盖`，不再显示 `ODS/DWD/ADS` 等表生命周期标签；下方显示字段列表 |
| 字段详情+编辑 | 点击字段行 | 右侧滑出抽屉，含 Monaco 表达式编辑器 |
| 上游 tooltip | hover 字段行 | 浮层显示该字段依赖的上游字段和表达式 |
| 新建表 | 左侧分类操作行右侧 [+ 新建表] | 弹窗: 表名/层级(大类→子类)/存储/标签/描述 + 初始字段定义列表 |
| 编辑表 | 表详情 [编辑表] | 右侧抽屉: 修改层/存储/主分类/标签/描述，表名只读 |
| 删除表 | 表卡片 [删除] | 二次确认 + 下游影响检查 |
| 新建字段 | 字段列表 [+ 新建字段] | 行内或弹窗添加 |
| 删除字段 | 字段行 ✕ | 下游依赖检查 → 警告或拒绝 |
| 导出 YAML | 查询结果栏 [导出 YAML] / 表详情 [导出单表] | 全量或按选定表导出到 metadata-yaml/ |
| YAML 预览 | 表卡片 [预览 YAML] | 只读弹窗展示当前表 YAML 内容 (Monaco 语法高亮)，不落盘 |
| 查看血缘 | 表卡片 [查看血缘] | 跳转到 `/metadata/lineage?table=xxx` |
| 创建下游表 | 表详情 [创建下游表] | 打开新建表弹窗，预填当前表的分类、标签、存储、`<当前表名>_downstream` 表名和来源描述 |

#### 分类与标签导航管理

`/metadata` 左侧导航从单一分层过滤升级为“主分类树 + 标签筛选”：

- 分类树单选：展示 `环境/设备/网络/用户/业务/源数据` 六个大分类及其小分类，节点显示表数量；树默认收起；点击分类节点时同时展开下一层，并作为主导航清空旧的搜索和标签条件，只按该分类过滤中间表列表；点击展开箭头也会同步过滤当前分类，点击表节点打开对应表详情。
- 标签多选：标签平铺展示，可按 `MetaTagGroup` 折叠；内置“表层级”通用标签组包含 `ODS/DWD/DWS/ADS/EVAL`，用于与主分类树正交过滤；多标签过滤固定要求全部匹配，不再暴露“任一/全部”切换。选中分类后再选择标签时，标签与当前分类按 AND 叠加；例如 `网络 + 覆盖` 只显示网络分类范围内同时拥有 `覆盖` 标签的表。
- 搜索组合：左侧搜索、分类、标签过滤条件同时生效，三者之间采用 AND；但用户重新点击分类树节点时会重置旧搜索和旧标签，避免历史筛选条件导致表列表为空或误判。页面不再提供独立层级下拉，`ODS/DWD/DWS/ADS/EVAL` 通过“表层级”标签统一筛选。
- 未归类治理：历史无分类表进入虚拟节点“未归类”；保存表编辑时必须选择真实叶子分类。
- 页面分栏：左侧面板只承载搜索、主分类树和标签筛选；查询出的表列表固定在中间面板；表详细信息固定在右侧面板。
- 表列表展示：表卡片显示主分类路径和标签 chips，便于确认当前纳管状态。

左侧分类树上方提供 `[全部]` 与右对齐的 `[+ 新建表] [管理分类]`，其中 `[全部]` 清空分类、搜索和标签条件，`[+ 新建表]` 打开新建表弹窗，“管理分类”打开抽屉：
...
表详情与表编辑抽屉新增“主分类路径 + 标签”区：

- 表详情信息卡片中的“层级”只展示主分类两级路径；`ODS/DWD/DWS/ADS/EVAL` 只作为标签 chips 展示，不再放在“层级”字段。
- 新建表表单的“层级”使用大类 → 子类两级 Cascader，只能选择启用状态的叶子分类，保存时写入隐藏的 `category_id`。
- 编辑表抽屉保留表名只读；主分类使用已治理分类选项维护，生命周期层级仍可通过表层级标签同步到 `Table.layer`。
...
- 新建表“层级”使用两级分类选择：第一级为 `环境/设备/网络/用户/业务/源数据`，第二级为对应子类；保存时写入 `category_id`。`ODS/DWD/DWS/ADS/EVAL` 继续作为“表层级”标签维护，并在保存时同步到 `Table.layer`；如果未显式选择表层级标签，沿用默认或既有层级策略，不把两级分类路径写入 `Table.layer`。
- [编辑表]：从表详情打开右侧抽屉，表名只读，维护存储、描述、主分类、标签和表生命周期层级；保存后刷新分类树计数、查询结果和表详情。
...
血缘页采用“左侧控制台 + X6 血缘画布 + 字段边抽屉”。默认先呈现表级血缘，避免字段级边过早铺满画布；用户展开表节点后，再在同一个节点内按字段行显示字段级血缘。

血缘工作台的中间画布使用 AntV X6，而不是复用 `/pipeline` 的 G6 DAG。X6 负责可编辑图工作台能力：表节点、字段端口、曲线边、字段级虚线边、边 hover/click、拖动端点重连和画布导航。左侧控制面板承载单选下拉根表选择、当前选中表卡片、展开层级、前向/后向开关、SQL 逻辑预览和 SQL 操作区；SQL 操作区分为“生成 SQL/同步到表定义”主操作行和“导入 SQL/新建血缘边/用 NL 修改”工作流操作行，SQL 内容紧随按钮区显示，告警和差异提示显示在 SQL 内容下方。字段级边编辑器采用 Ant Design Drawer，默认不显示，点击字段级血缘边或从一个字段端口连到另一个字段端口后滑出。
...
rectangle "左侧控制台" as left {
  component "根表单选下拉\n当前选中表卡片" as search
  component "展开层级\n前向/后向 checkbox" as filters
  component "SQL 逻辑\n主操作行\n工作流操作行\nSQL 内容\n告警/差异提示" as sql
}
rectangle "X6 血缘画布" as canvas {
  component "表级节点与曲线边" as tableGraph
  component "展开字段行\n淡色虚线字段边" as fieldGraph
  component "右键菜单\n滚轮缩放\nMini-map" as graphTools
}
rectangle "字段级血缘边 Drawer\n默认隐藏" as drawer {
  component "计算类型\n参数\n表达式\n保存/删除" as edgeEditor
}
left --> canvas : 查询/过滤/操作
canvas --> drawer : 点击字段级边
drawer --> left : 保存后刷新 SQL 预览
@enduml
```
...
| 字段级边 | X6 edge + `DERIVES_FROM` data | 淡色虚线曲线，使用简洁线性单向箭头从 `source field` 指向 `target field`，不显示粗大的 arrowhead 工具；仅当源表或目标表展开后显示 | hover 只显示转换表达式；click 打开字段边抽屉；端点可重连到其它字段 port |

#### 交互清单

| 交互 | 操作 | 效果 |
|------|------|------|
| 定位中心表 | URL param `?table=` 或左侧 `Select showSearch` 单选下拉 | 下拉框输入时实时过滤候选表；选中表后下方当前表卡片切换为该表，血缘图围绕该表重新查询、布局并高亮居中 |
| 控制方向 | 在“展开层级”同一行勾选/取消 `前向`、`后向` checkbox | 隐藏或显示目标表的下游/上游表节点和表级边 |
| 展开字段 | 点击表节点 `+` 小按钮 | 表节点变高，在节点内分行显示所有字段和字段锚点；同列上游/下游节点按展开后的节点高度重排，避免字段列表覆盖相邻表 |
| 字段级血缘显示 | 展开相关表节点 | 在字段锚点之间显示淡色虚线曲线 |
| 边 hover | 鼠标移动到表级边或字段级边 | 字段级边使用浅色 tooltip，只展示转换表达式，避免遮挡和信息过载 |
| 边 click | 点击字段级边 | 打开字段级血缘边抽屉，显示计算类型、参数和表达式编辑器；未选中字段边时抽屉不显示 |
| 新建字段级连线 | 从一个表字段 `out:<field>` 端口连到另一个表字段 `in:<field>` 端口 | 不静默丢弃新边；解析源/目标字段后打开字段级血缘边抽屉，预填源字段、目标字段、默认 `DIRECT` 计算类型和转换表达式，保存后创建 `DERIVES_FROM` 并刷新图和 SQL 预览 |
| 锚点拖动 | 拖动字段级边的源/目标锚点到其他字段 | 即时调用后端保存新端点；成功后刷新图和 SQL 预览，失败则回滚 |
| 图编辑联动 SQL | 新建/删除/改边、改计算类型、拖动端点 | 自动重新生成当前目标表 SQL 预览 |
| SQL 写回（第一版） | 在“导入 SQL”抽屉确认应用 | 将导入 SQL 写入 `Table.sql_logic` 并追加 `Change`；左侧 SQL 逻辑区的“同步到表定义”按钮作为后续直接写回入口 |
| 导入 SQL | 点击“导入 SQL”并粘贴 `SELECT ... FROM ...` | 解析后展示字段/血缘变更预览；确认后第一版写入 `Table.sql_logic` 和 `Change`，字段/血缘变更应用为后续扩展 |
| 画布右键菜单 | 右键表节点、边或空白画布 | 表节点菜单包含“创建下游表”；点击后跳转 `/metadata?table=<表名>&create_downstream=1`，元数据页自动打开下游表创建弹窗；点击菜单外区域或按 Esc 后菜单关闭 |
| 画布导航 | 拖拽、滚轮缩放、Mini-map | 支持大图定位和局部查看；鼠标滚轮直接缩放 X6 画布，不要求按住 Ctrl/Meta；全屏、全部折叠、展开当前表、刷新图按钮作为后续扩展 |
...
| `selectedEdge` | selected X6 edge | 选中字段边高亮，并与字段边抽屉同步 |
...
| `node:contextmenu` | 在鼠标位置打开右键菜单；表节点菜单可触发创建下游表 | 创建下游表跳转到 `/metadata` 后由现有 `POST /api/tables` 保存 |
| `edge:contextmenu` / `blank:contextmenu` | 打开边或画布右键菜单；边可进入编辑，画布可进入新建操作 | 取决于菜单动作 |
| 展开按钮 click | 更新 React `expandedTables`，重新计算 X6 graph data | 无 |
| `edge:mouseenter` / `edge:mouseleave` | 展示/隐藏 tooltip | 无 |
| `edge:click` | 若为字段级边，调用 `onSelectFieldEdge(edge)` 并打开字段边 Drawer | 无 |
...
SQL 逻辑不存放在 X6 cell data 中。左侧 `LineageSqlPanel` 通过 `/api/lineage/sql/preview` 读取生成结果；字段级边编辑、端点重连、新建/删除边成功后，统一刷新 `lineage-graph` 与 `lineage-sql-preview`。
...
- SQL 预览不完整：允许继续编辑图；第一版写入 `Table.sql_logic` 需通过“导入 SQL”确认应用，左侧 SQL 逻辑区的“同步到表定义”直接写回为后续扩展。
- 并发冲突：前端随应用请求携带预览时的图版本；后端版本冲突校验作为后续扩展，落地后提示刷新重试。
- 图编辑导致 SQL 改变：左侧 SQL 逻辑区展示“生成 SQL”和“表上已保存 SQL”的差异提示。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/tables` | 表列表 (支持 ?layer= ?storage= ?search= ?category_id= ?include_children=true ?tag_ids= ?tag_match=any/all；默认 all) |
...
`GET /api/tables` 过滤语义以 UI 当前行为为准：分类、搜索、标签之间是 AND；`tag_ids` 多值默认全部匹配；父分类配合 `include_children=true` 覆盖其直接子类，二级分类按叶子精确过滤。前端元数据页不暴露 `tag_match=any`，但后端保留该参数作为兼容扩展。
...
| # | 用例 | 步骤 | 预期结果 |
|---|------|------|----------|
| P3-1 | 元数据浏览页面: 分类树+标签+搜索 | 打开 `/metadata` → 左侧分类树默认收起 → 点击 `网络` 显示网络域 7 张表示例 → 点击 `网络 / 覆盖` 显示 2 张覆盖表示例 → 再选择 `覆盖` 标签仍只显示覆盖类表；点击 `[全部]` 后搜索框输入 "会话" | 中间表列表按分类、标签、搜索 AND 过滤；搜索结果定位到 `dwd_session_qos`；点击表行后右侧详情“层级”显示两级分类路径，字段列表含 avg_rsrp/avg_sinr 等 |
| P3-2 | 元数据浏览: 字段详情+上游 tooltip | 点击 `dws_cell_hourly` → hover `avg_rsrp` 字段 | tooltip 浮层显示上游: dwd_session_qos.avg_rsrp，表达式: AVG(rsrp) |
| P3-3 | 元数据维护: 新建表 | 点击 [+ 新建表] → 填写表名 → 选择大类/子类两级分类 → 选择存储和标签 → 添加 5 个字段 → 保存 | Neo4j 写入 Table 节点、`IN_CATEGORY/TAGGED_WITH` 关系、5 个 Field 节点和 5 条 HAS_FIELD 边成功，GET /api/tables 新增 1 条，metadata-yaml/ 下生成对应 .yaml |
...
| P3-9 | 血缘边结构化编辑 | 点击字段级边 → 字段边抽屉打开 → 修改计算类型为 `AGGREGATE` 并填写参数 → 保存 | `DERIVES_FROM` 更新 `calc_type/calc_params/transform_expr`，图刷新，左侧 SQL 预览同步变化；未选中字段边时抽屉不显示 |
| P3-10 | 拖动锚点改血缘端点 | 在 X6 画布中拖动字段级边源锚点到另一个上游字段 port | 后端即时保存新端点并做循环校验；成功后图与 SQL 预览刷新，失败时连线回滚 |
| P3-10a | 基于血缘生成 SQL | 选中目标表 → 点击「生成 SQL」或完成一次血缘编辑 | 左侧 SQL 逻辑区展示当前目标表直接上游 Spark/Hive SELECT SQL；第一版“同步到表定义”按钮展示占位提示，直接写回为后续扩展 |
